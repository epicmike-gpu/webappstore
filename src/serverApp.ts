...
import { WebApp, SafetyCheckResult } from './types';
...
export function createApiRouter() {
  ...
  router.get('/v1/apps/categories', (req, res) => {
  ...
  // Get featured apps
  router.get('/v1/apps/featured', (req, res) => {
    const version = (req.query.version as string) === 'overseas' ? 'overseas' : 'cn';
    const allApps = [
      ...(version === 'overseas' ? overseasWebApps : cnWebApps),
      ...customApps[version],
  ...
  router.post('/v1/apps/safety-check', async (req, res) => {
    const { url } = req.body;
  ...
  router.post('/v1/apps/extract-logo', async (req, res) => {
    const { url } = req.body;
    if (!url || typeof url !== 'string') {
      return res.status(400).json({ success: false, error: 'URL is required' });
    }
